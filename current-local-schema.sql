...
CREATE POLICY "Admins can manage posts" ON "public"."posts" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."admin_users"
  WHERE (("admin_users"."id" = "auth"."uid"()) AND ("admin_users"."is_active" = true)))));
...
CREATE POLICY "Anyone can create analytics events" ON "public"."analytics_events" FOR INSERT TO "anon" WITH CHECK (true);
...
CREATE POLICY "Anyone can submit career application" ON "public"."career_applications" FOR INSERT TO "anon" WITH CHECK (true);
...
CREATE POLICY "Everyone can view published posts" ON "public"."posts" FOR SELECT USING (("status" = 'published'::"text"));
...
CREATE POLICY "Users can create own profile" ON "public"."user_profiles" FOR INSERT WITH CHECK (("auth"."uid"() = "id"));



CREATE POLICY "Users can enroll in courses" ON "public"."course_enrollments" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = "auth"."uid"()));
